...
def single_link_merge(c1_index, c2_index, X_matrix, clusters_matrix, distance_col, cluster_col, distances_matrix):
    ...
    cluster_indexes, cluster_items = [], []

    c1_index = X_matrix[c1_index, -1]
    c2_index = X_matrix[c2_index, -1]

    for i in range(num_points):
        if X_matrix[i, -1] == c1_index or X_matrix[i, -1] == c2_index:
            cluster_indexes.append(i)
            if X_matrix[i, -1] == c2_index:
                X_matrix[i, -1] = c1_index

    for item in X_matrix:
        if item[-1] == c1_index or item[-1] == c2_index:
            cluster_items.append(item[:-1])

    for i in range(num_points):
        if i != c1_index and i != c2_index:
            new_distance = euclid_dist(cluster_items[0], X_matrix[i, :-1])
            for item in cluster_items:
                if euclid_dist(item, X_matrix[i, :-1]) < new_distance:
                    new_distance = euclid_dist(item, X_matrix[i, :-1])
...
def average_link_merge(c1_index, c2_index, X_matrix, clusters_matrix, distance_col, cluster_col, distances_matrix):
    ...
    for i in range(num_points):
        if i != c1_index and i != c2_index:
            mean = 0.0
            for item in cluster_items:
                mean = mean + euclid_dist(item, X_matrix[i, :-1])
            mean = mean / len(cluster_items)
            clusters_matrix[i, c1_index] = mean
            clusters_matrix[c1_index, i] = mean
...
# bottom_up_clustering(single_link_merge, X, distances, threshold=7)
# bottom_up_clustering(complete_link_merge, X, distances, threshold=7.5)
bottom_up_clustering(average_link_merge, X, distances, threshold=9.5)
